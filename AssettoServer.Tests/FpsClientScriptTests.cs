...
public sealed class FpsClientScriptTests
{
    ...
    public void CarrierCars_PreserveNativeParticipantStateAndDisableNativeLeaderboard()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(script, Does.Contain("ASRC_FpsBoundary"));
            Assert.That(script, Does.Contain("outOfBoundsRemaining"));
            ...
            Assert.That(script, Does.Contain("190 * hudScale"));
            ...
            Assert.That(script, Does.Contain("modernAssetRevision = 14"));
            ...
            Assert.That(script, Does.Contain("/fps/assets/asrc-fps-modern-v14.zip"));
            ...
            Assert.That(script, Does.Contain("directory = 'extension/audio/asrc_fps/'"));
            Assert.That(script, Does.Contain("fire_assault_rifle_01.wav"));
            Assert.That(script, Does.Contain("fire_compact_smg_01.wav"));
            Assert.That(script, Does.Contain("fire_desert_eagle_01.wav"));
            Assert.That(script, Does.Contain("fire_colt_1911_01.wav"));
            Assert.That(script, Does.Contain("fpsAudio.playWeaponFire(message.weaponType"));
            Assert.That(shotDefinition, Does.Contain("fpsAudio.playImpact(message.impact, point)"));
            Assert.That(script, Does.Contain("fpsAudio.playHurt(actors[message.victimID])"));
            Assert.That(script, Does.Contain("fpsAudio.playDeath(actors[message.victimID])"));
            Assert.That(script, Does.Contain("fpsAudio.playGrenadeExplosion(message.type, origin)"));
            Assert.That(script, Does.Contain("pickup_magazine.wav"));
            Assert.That(script, Does.Contain("kill_confirm.wav"));
            Assert.That(script, Does.Contain("effectClock - (actor.audioHurtAt or -10) < 0.3"));
            ...
            Assert.That(script, Does.Contain("event.cameraTrackMultiplier = 1"));
            Assert.That(script, Does.Contain("fpsAudio.logPlaybackFailure(sound.fileName, 'post-start-validity'"));
            Assert.That(script, Does.Contain("audio playback failed: file="));
            Assert.That(script, Does.Contain("maxActive = 64"));
            Assert.That(script, Does.Contain("while #fpsAudio.active >= fpsAudio.maxActive"));
            Assert.That(script, Does.Contain("fpsAudio.snapshotTransition(actor"));
            Assert.That(script, Does.Contain("fpsAudio.update(dt)"));
            Assert.That(script, Does.Contain("previousGameplayActive == true and not gameplayActive"));
            Assert.That(script, Does.Not.Contain("extension/audio/asrc_fps/rifle.wav"));
            ...
            Assert.That(script, Does.Contain("function fpsVisual.isHostileGrenade"));
            Assert.That(script, Does.Contain("grenade.ownerID == localSessionID"));
            Assert.That(script, Does.Contain("owner.team == localActor.team"));
            Assert.That(script, Does.Contain("<= 18 * 18"));
            Assert.That(script, Does.Contain("function hud.drawGrenadeIndicators"));
            Assert.That(script, Does.Contain("render.projectPoint"));
            Assert.That(script, Does.Contain("hud.drawGrenadeIndicators(size, hudScale)"));
