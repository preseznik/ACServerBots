...
`JoinSlotSelection` controls which eligible `AI=auto` slot is claimed when several slots use the player's selected car model. `First` uses the lowest configured `CAR_n`, `Last` uses the highest, and `Random` chooses independently for each connection. The default is `First`. Occupied and ineligible slots are skipped, `AI=fixed` slots remain protected, GUID-reserved slots retain priority, and an explicit CSP `SLOT_INDEX` request still wins. Before a race starts, this controls the player's grid position. During a race, it selects the active bot being replaced at that bot's current position; it does not teleport a joining player back to the starting grid.
...
Road geometry is no longer treated as a single centre-height approximation. A cached, lightweight preflight first rejects unreadable, truncated, open, or unsupported race splines and missing physical `MODEL_*` references before starting physics preparation. Physics preparation rasterizes the selected `fast_lane.ai` into a bank-aware height corridor, then rejects physical triangles from other decks, undersides, adjacent roads, and steep wall-like surfaces. Preparation reports source/selected triangle counts and centerline coverage and fails closed below 70 percent coverage. AC grid transforms are grounded on that route surface first; if an authored transform misses a triangle edge, its position may snap at most 6 metres to the selected route or 3 metres to a same-height layout fallback while preserving its heading. Preparation reports the snap count and maximum distance. A legal staggered grid column outside the retained triangle samples may otherwise use the selected layout's physical road only when it is within 1.5 metres below `AC_START_n`. A fallback slot contributes only a direction-limited, height-bounded launch apron to the final runtime collider, and preparation validates the exact saved triangles under a representative four-wheel footprint through the first 25 metres. This supports stock layouts such as Vallelunga Classic without importing the layout's complete raw road mesh or allowing a missing upper deck to snap to unrelated road geometry below it. Single-level layouts keep their route-matched KN5 road and nearby wall-like barrier faces. If the selected corridor contains materially separated physical decks, preparation projects the spline centreline back onto the matching physical road deck before building its continuous banked suspension ribbon, then suppresses the ambiguous raw barrier mesh; lane bounds and recovery remain active, while car-to-car and human-to-bot contacts are unchanged. The projected route is also serialized as a surface atlas. Physical wheel raycasts remain authoritative, but the atlas selects the expected nearby deck, rejects hits over 1.5 metres from it, and bridges at most 200 ms of contact loss after an established physical contact. It never fabricates first contact or reaches beyond the authored lane plus a two-metre edge margin. The height correction is smoothed without replacing the spline's authored bank normal. A one-metre non-raceable suspension shoulder sits outside the authored lane bounds so a wheel cannot fall through an artificial ribbon edge during a bounded correction; AI planning never treats that shoulder as passing room. This prevents multilevel mod tracks from feeding duplicate floors or invisible collision walls into the chassis solver without making the car follow an inaccurate spline height. Each bot keeps one dynamic chassis and raycasts the prepared support road below every installed wheel transform. Independent spring and damper impulses are applied at those four contact points, with compression and extension bounded from the actual front/rear radii in `tyres.ini`. The launcher reads unpacked `data` first and otherwise decodes only `tyres.ini` and `car.ini` from `data.acd` in memory; installed files are never extracted or modified. The network reference height compensates for AC's client-side `GRAPHICS_OFFSET` and the difference between visual wheel-node height and actual tyre radius; this changes only the transmitted render reference, not the physical chassis. Sampling across each tyre is selected by fidelity, while a slope-aware residual filter rejects one-frame triangle-edge steps. The chassis does not collide directly with drivable triangle soup, so KN5 seams cannot launch it. Network position comes directly from the resulting chassis pose and the per-model AC reference height—there is no per-tick vertical snap or road clamp. Recovery relocation is reserved for a non-driveable roll attitude, implausible excess vertical velocity, material off-course state, or a chassis that has lost wheel support below the physical road. Attitude is measured relative to the sampled bank normal rather than world-up, so ordinary banking does not look overturned. A cooldown prevents a malformed surface from causing a teleport loop, while a deeply unsupported chassis or complete inversion can bypass that cooldown before falling through the world. A recovery scans forward and laterally for a target supported by the final runtime collider, projects the requested heading onto that road plane, and aligns the chassis with its bank normal; if no supported target exists yet, it waits instead of reusing the falling car's current height. Fidelity changes solver iterations, substeps, threading, chassis CCD, and tyre ray sample count without changing the suspension tune. This is still not the proprietary Kunos vehicle simulation. Longitudinal and lateral tyre forces are supplied by the race controller from installed car metadata; the server does not reproduce Kunos tyre slip curves, detailed aero, damage, pit strategy, weather adaptation, or offline AI behavior. Connected humans remain authoritative on their clients and therefore act as kinematic obstacles in the server world: bots receive contact response, but the server cannot apply the equal collision impulse back into an unmodified human client. Mixed models are supported, but control-profile quality depends on installed car metadata.
...
Passing-lane tracking is deliberately separate from off-course protection. The drive limiter receives only the physical chassis overrun beyond the current spline corridor; distance from the racing line or from a newly selected passing lane cannot cut throttle. A committed lane command is also held within a speed-bounded 1.25-to-2.25-metre lead of the chassis, so a rapid planner update cannot run away from the car, remove its drive, and freeze the lane transition. Emergency overlap braking retains a bounded crawl target instead of commanding a permanent zero-speed queue.

Crossing the physical lane-centre corridor applies a progressive off-road response immediately. At two metres of overrun the automatic target-speed scale reaches 45 percent and planar rolling resistance reaches 3 m/s². The steering target is clamped farther inside the authored road widths and uses a shorter intercept until the chassis returns, while at least 35 percent drive authority is retained so a car in grass or sand does not become a permanent stopped obstacle. This uses `fast_lane.ai` road widths as the portable pavement boundary because the prepared collision asset does not retain KN5 material names; it does not claim per-material Kunos grass, gravel, or sand fidelity. Network steering telemetry reverses the rigid-body positive-yaw sign to match AC's visual wheel convention.

A stopped obstruction activates predictive corridor planning. The bot evaluates several legal centres on both sides over a 2.5-to-4-second horizon, predicts the longitudinal and lateral paths of every nearby bot and human, and rejects any candidate that overlaps a collider envelope. Remaining candidates are scored for interaction risk and steering distance; overlapping stopped-queue reservations are excluded. Before commitment, five points along the selected path must also have a prepared physical road surface within 0.75 metres of the authored spline height. A bot with fewer than two grounded wheels, more than 1.25 metres of surface-height error, or an active recovery cooldown is excluded from obstacle, pass, and reservation planning; a pass committed against such a car is cancelled immediately. Explicitly STOP-controlled cars remain valid grounded obstacles and receive the extended lookahead, while ordinary moving-car passes retain the proven racing trigger and selector so dense-field recovery does not perturb stable high-speed behavior.
...
Race Control also supports `Spectator` grid entries. They render as `SPECTATOR_MODE=1` with `AI=none`, remain available as CSP camera-only network slots, and are excluded from bot simulation, physical grid capacity, timing, classification, DNF handling, and race-over checks. Spectator rows are always staged after racing rows so they cannot displace a valid `AC_START_n` position.
...
`Difficulty` and `Aggression` must be in `0..1`, the update rate must be `10..120`, the start point must belong to a closed usable spline, and the prepared asset must contain enough contiguous `AC_START_0..n` transforms for the roster. Physical-road preparation only switches to a synthetic spline ribbon after sustained multilevel evidence; a local overlap is not sufficient. Every final grid pose and its launch footprint are validated against the collider actually written to disk, with a bounded layout-mesh support apron added where needed. `Fidelity` selects solver iterations, substeps, worker threads, and continuous collision detection: `Efficient` is the lowest CPU setting, `Balanced` is the default, and `High` enables swept continuous collision detection with the strongest solver. All levels use the same exact track triangles and car colliders. Vehicle profile models must be unique and their physical inputs are bounded during configuration validation. Race mode also requires visible AI and a private IPv4 listener. Dynamic hourly traffic density cannot be combined with race mode. Mid-race takeover and first-human restart are disabled by default in generic configuration; the CM LAN stager enables both when bots are active. Takeover additionally requires `RACE IS_OPEN=1` and at least one `AI=auto` slot.

`DifficultyVariancePercent` and `AggressionVariancePercent` accept `0..100`. Race mode deterministically resolves independent per-slot skill and aggression around the configured baselines, clamped to `0..1`, so the same entry list produces the same personalities after a restart. Optional entry-list keys `AI_DIFFICULTY` and `AI_AGGRESSION` override the resolved value for one slot; `-1` or an omitted key keeps automatic variance. Per-slot skill is used everywhere the race planner applies pace or cornering difficulty, and per-slot aggression is used throughout obstacle discovery, predictive passing, pass commitment and extension, yielding, and passing speed.
...
Each run writes `events.jsonl`, `samples.jsonl`, and `summary.json`. Samples include a session generation plus each bot's lap, spline point, position, velocity, requested and effective target speed, physical corridor overrun, course-drive scale, line offset, obstacle distance, steering, slip, road-height error, suspension compression, grounded-wheel count, raw-surface discontinuity count, actual upward speed, slope-relative excess upward speed, upright state, recoveries, pass state, and pass counters. Session changes snapshot the completed race before statistics are cleared, so a later practice session cannot overwrite race classification with zeroes. Accepted STOP/GO commands are preserved as explicit stopped-obstacle episodes with the stopped slot, duration, committed and completed passes, and contact manifolds. Aggregate counters remain cumulative across session restarts. Summary schema 4 records classification, run-wide physics maxima, contacts, anomalies, simulated/wall duration, target and achieved real-time factors, travelled vehicle-kilometres, and recoveries, contact episodes, and full stops per 100 vehicle-kilometres both per car and across the field. These normalized rates make dense and sparse grids and short and long circuits directly comparable.
...
For a diagnostic pass over every installed layout that has models, an AI line, and at least two pits, use one deterministic full-grid run per layout and continue after bad mod content or native failures:

```powershell
.\tools\Test-RaceBotsMatrix.ps1 -AllUsableTracks -FullGrid -RaceLaps 1 `
  -Seeds 1 -MaximumSimulatedMinutes 20 -MaximumWallSeconds 120 -ContinueOnError
```

Layouts that cannot be simulated are retained in `preflight-unsupported.json` and in the report rather than silently skipped.
