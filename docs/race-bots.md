...
All bots are released on the same race-start tick. A shared half-second launch window prevents stationary grid rows from creating a one-car-at-a-time start. Each bot retains the lateral position of its authored grid transform and holds lane transitions for the first two seconds, avoiding a sideways convergence immediately after release. Collision avoidance then resumes, while new pass commitments remain disabled for four seconds so each rigid body can settle from its staggered grid transform onto its assigned base line.
