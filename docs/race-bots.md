...
All bots are released on the same race-start tick. A shared half-second launch window prevents stationary grid rows from creating a one-car-at-a-time start. Each bot retains the lateral position of its authored grid transform and drives forward on that grid line for at least 25 metres. It then blends toward its independent racing line at half the normal line-change rate, pausing the merge whenever another participant occupies the nearby target corridor. Collision avoidance resumes after the shared launch window, while new pass commitments remain disabled for four seconds.
...
Race-line and pass decisions are participant-neutral. Nearby bot positions are compared in the shared spline-relative lane frame, while connected humans continue to use direct world-space projection. A stopped car uses the best safe escape corridor available; a moving pass waits for a full corridor and remains committed to its chosen side as both cars' independent base lines drift. The pass is recorded at 3.1 metres of authoritative separation, but the passer's speed headroom and the overtaken bot's bounded lift do not begin until a 4-metre clearance has remained stable for two seconds; a 3.8-metre reset threshold prevents steering noise from chattering the timer. The passer then retains a minimum conversion pace so the maneuver cannot degrade into a matched-speed traffic queue. All lateral movement still goes through steering and tyre forces.
...
Automated acceptance covers configuration, exact grid-transform handling, prepared-geometry serialization and winding, countdown holding, forward-distance and occupancy-gated grid merging, forward lap wraps, wrong-way/double-crossing rejection, roster claims/freeze, mid-race takeover eligibility and fresh results, first-human restart gating/re-arming, DNF policy, classification ordering, packet rows, cornering, following, continuous line variation, stopped-car escape, target-relative committed passing, pair cooldowns, collision recovery, mixed-model staging, profile-driven acceleration, bounded top speed, braking, gears, and RPM. Prepared track physics accepts AC's numbered physical meshes and explicitly named wall/collision meshes; visual geometry that merely shares a `surfaces.ini` prefix is excluded, and duplicate triangles are collapsed before the Bepu mesh is built. Headless passing acceptance requires a measured lane change, at least 3.1 metres of authoritative bot-to-bot separation, a completed pass, and bounded vehicle-contact frames; it also records maximum upward speed so a short-lived collision launch cannot pass between five-second log samples.
