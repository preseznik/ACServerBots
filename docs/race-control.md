...
Client pack version 6 includes one background-loaded **ASRC FPS HUD** app. A versioned local `ac.connect` bridge exposes presentation state only; the app uses CSP's exclusive-HUD callback to replace normal AC UI and other Lua/Python apps during active FPS gameplay, then yields in pre-match menus, results, replay, and non-FPS sessions. The server-delivered online script owns the FPS pause surface: it replaces AC/CSP's racing-oriented pause screen with a **MATCH MENU**, live standings, **RETURN TO MATCH**, **ASSETTO CORSA OPTIONS**, and a confirmed **LEAVE SERVER** action. Selecting options deliberately yields to the native menu so video, audio, controls, and CSP settings remain accessible; resuming and pausing again restores the match menu. If the app heartbeat is missing, stale for 0.5 seconds, or version-incompatible, the embedded online script immediately resumes the same complete gameplay HUD. The app also adds a 40 m player-up combat radar: opponents appear only while track geometry has line of sight or for two seconds after an authoritative shot. Version 6 corrects the radar's horizontal handedness and decodes fixed-size bridge strings before drawing player names and kill-feed entries. Content Manager presets, desktops, and user app configuration are not modified.
