...
The gate runs over one long AC Practice session. Every participant occupies a normal AC carrier slot, while the embedded CSP online script immobilizes and hides carrier cars, owns the camera, captures keyboard/mouse or Xbox-controller input, renders the locally installed pit-crew prototype, and draws the crosshair, health, clock, kill feed, scoreboard, and final ranking. Press `F6` to switch between first-person and a collision-clipped over-the-shoulder view. The authoritative capsule walks slopes up to 45 degrees and steps onto walkable stair or kerb tops up to 0.48 m; larger drops enter a gravity-driven fall, while taller geometry remains solid and requires jumping, vaulting, or mantling. The server runs a separate fixed-step `FpsWorld`: inputs are authenticated to the sender's assigned slot, stale or impossible inputs are rejected, actor snapshots (including the current landing surface) are sent at 20 Hz, local motion is predicted and reconciled, firing and damage are server-authoritative, and reliable events carry hits, kills, score, and match state. No client-reported hit is accepted and no `acs.exe` hook is installed.
