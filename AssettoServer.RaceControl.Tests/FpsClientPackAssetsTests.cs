...
public sealed class FpsClientPackAssetsTests
{
    ...
    public void HudAppAssetsUseTheVersionedBridgeAndOwnFileAudioPlayback()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(script, Does.Contain("drawGrenadeIndicators(size, scale)"));
            ...
            Assert.That(script, Does.Contain("pickupPrompt = ac.StructItem.string(72)"));
            Assert.That(script, Does.Contain("drawPickupPrompt(size, scale)"));
            Assert.That(script, Does.Contain("center.y + 188 * scale"));
            Assert.That(script, Does.Contain("ui.dwriteDrawTextClipped(text, 23 * scale"));
            Assert.That(script, Does.Not.Contain("panel(p1, p2, scale, 0.88)"));
    ...
    public async Task ClientPackV52ContainsTeamOperatorsSkinsAndLocomotion()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(FpsClientPackBuilder.ClientPackVersion, Is.EqualTo(53));
            ...
            Assert.That(FpsClientPackBuilder.DefaultFileName,
                Is.EqualTo("asrc-fps-compatibility-client-v53.zip"));
            ...
            foreach ((string path, byte[] data) in FpsClientPackAssets.GetLoadoutImages())
            {
                Assert.That(entries.Keys, Does.Contain(path));
                Assert.That(ReadEntry(entries[path]), Is.EqualTo(data), path);
            }
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(root.GetProperty("clientPackVersion").GetInt32(), Is.EqualTo(53));
    ...
    public void ModernAssets_AreCompleteAndWithinProjectOwnedDirectory()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(root.GetProperty("operatorAnimations").EnumerateObject().Count(), Is.EqualTo(43));
            ...
            foreach (string name in new[] { "crouch_fire", "crouch_reload", "prone_fire", "prone_reload" })
            {
                JsonElement entry = root.GetProperty("operatorAnimations").GetProperty(name);
                Assert.That(entry.GetProperty("trackCoverage").GetString(), Is.EqualTo("upperBody"));
                Assert.That(entry.GetProperty("trackCount").GetInt32(), Is.EqualTo(56));
            }
            Assert.That(root.GetProperty("operatorAnimations").GetProperty("land")
                .GetProperty("loop").GetBoolean(), Is.False);
            Assert.That(root.GetProperty("operatorAnimations").GetProperty("airborne")
                .GetProperty("sourceAction").GetString(), Is.EqualTo("Jump_Loop"));
