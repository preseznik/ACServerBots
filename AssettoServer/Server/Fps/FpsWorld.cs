...
public sealed class FpsWorld : IHostedService
{
    ...
    private readonly Dictionary<byte, (FpsOperatorModel Model, FpsOperatorSkin Skin)> _knownAppearances = [];
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var actor in _simulation.Actors.Where(actor => actor.Active).OrderBy(actor => actor.Id))
        {
            ...
            Log.Debug(
                "FPS actor initial spawn: actor={ActorId}, role={Role}, human={Human}, spawn={SpawnCount}, position={Position}, yaw={Yaw:F3}, health={Health}, operator={OperatorModel}, team={Team}, skin={OperatorSkin}",
                actor.Id, actor.Role, actor.HumanControlled, actor.SpawnCount, actor.Position,
                actor.Yaw, actor.Health, actor.OperatorModel, actor.Team, actor.OperatorSkin);
        }
    ...
    private void OnLoadoutSelect(ACTcpClient client, FpsLoadoutSelectPacket packet)
    {
        ...
        lock (_sync)
        {
            ...
            var result = _simulation.SelectLoadout(client.SessionId, loadout,
                (FpsOperatorModel)packet.OperatorModel, (FpsOperatorSkin)packet.OperatorSkin);
            client.SendPacket(new FpsLoadoutResultPacket
            {
                ...
                OperatorSkin = packet.OperatorSkin,
            });
            if (result == FpsLoadoutResultCode.Applied)
            {
                ...
                _knownAppearances[actor.Id] = (actor.OperatorModel, actor.OperatorSkin);
                BroadcastMatch();
            }
            client.Logger.Information(
    ...
    private static FpsRosterPacket CreateRosterPacket(FpsActorState actor) => new()
    {
        ...
        OperatorSkin = (byte)actor.OperatorSkin,
    };
    ...
    private void BroadcastLoadoutChanges()
    {
        ...
        foreach (var actor in _simulation.Actors.Where(actor => actor.Active)
                     .OrderBy(actor => actor.Id))
        {
            var appearance = (actor.OperatorModel, actor.OperatorSkin);
            if (!_knownAppearances.TryGetValue(actor.Id, out var knownAppearance)
                || knownAppearance != appearance)
            {
                _knownAppearances[actor.Id] = appearance;
                Broadcast(CreateRosterPacket(actor));
            }
    ...
    private void SendLoadoutCatalog(ACTcpClient client)
    {
        ...
        var theme = _configuration.Extra.Fps.Theme;
        var matchType = _configuration.Extra.Fps.MatchType;
        var team = _simulation?.Actors.FirstOrDefault(actor => actor.Id == client.SessionId)?.Team
            ?? FpsTeamAssignment.Auto;
        client.SendPacket(new FpsLoadoutCatalogPacket
        {
            ...
            AllowedOperatorModels = FpsOperatorModels.AllowedMask(theme, matchType, team),
            DefaultOperatorModel = (byte)FpsOperatorModels.DefaultModel(theme, matchType, team),
            AllowedOfficerSkins = FpsOperatorModels.AllowedSkins(theme, FpsOperatorModel.Officer),
            AllowedGhostSkins = FpsOperatorModels.AllowedSkins(theme, FpsOperatorModel.Ghost),
            DefaultOperatorSkin = (byte)FpsOperatorSkin.Standard,
        });
    }
