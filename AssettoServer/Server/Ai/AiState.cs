...
public class AiState : IDisposable
{
    ...
    public int StoppedObstaclePassCommitCount { get; private set; }
    public int StoppedObstaclePassCompletedCount { get; private set; }
    ...
    private int _racePersonalitySeed;
    ...
    private bool _gridLineMergeBlocked;
    private bool _gridPathBlendStarted;
    private float _gridLaunchDistanceMeters;
    private float _gridLaunchLateralOffsetMeters;
    private float _gridPathBlend;
    private Vector3 _gridLaunchOrigin;
    private Vector3 _gridLaunchForward;
    ...
    private bool _stoppedObstaclePass;
    private float _requiredPassSeparationMeters = RaceBotMath.MinimumPassingSeparationMeters;
    private float _passAccelerationSeparationMeters = RaceBotMath.MinimumPassAccelerationSeparationMeters;
    private float _committedPassLateralOffsetMeters;
    private float? _alternatePassLateralOffsetMeters;
    private long _lastPassContactManifoldCount;
    private long _passContactSince;
    private long _lastPassContactAt;
    private byte? _raceContactTargetSessionId;
    private long _raceContactPersistenceUntil;
    ...
    private readonly record struct RaceObstacleCandidate(RaceParticipantSnapshot Participant,
        float ClearanceMeters, float LongitudinalMeters, float LateralOffsetMeters,
        float RelativeLateralMeters, float HalfWidthMeters);
    ...
    private readonly record struct RaceObstacle(RaceParticipantSnapshot Participant, float ClearanceMeters,
        ...
        RaceParticipantSnapshot PassTarget, float PassTargetLongitudinalMeters,
        float EnvelopeMinimumEdgeMeters, float EnvelopeMaximumEdgeMeters,
        bool IsStoppedCluster, int ClusterSize, byte[] ClusterSessionIds);
    ...
    private void SetRandomSpeed()
    {
        ...
        InitialMaxSpeed = _configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race
            ? configuredMaxSpeed * RaceBotMath.GridPaceFactor(
                _configuration.Extra.AiParams.MaxSpeedVariationPercent, _racePersonalitySeed)
            : configuredMaxSpeed + fastLaneOffset - (variation / 2) + (float)_random.NextDouble() * variation;
        if (_configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race)
        {
            InitialMaxSpeed *= RaceBotMath.PaceFactor(EntryCar.RaceDifficulty);
        }
    ...
    public void Teleport(int pointId, RaceGridPose? gridPose = null)
    {
        ...
        _racePersonalitySeed = _configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race
        ...
        _stoppedObstaclePass = false;
        _requiredPassSeparationMeters = RaceBotMath.MinimumPassingSeparationMeters;
        _passAccelerationSeparationMeters = RaceBotMath.MinimumPassAccelerationSeparationMeters;
        _committedPassLateralOffsetMeters = _baseLateralOffsetMeters;
        _alternatePassLateralOffsetMeters = null;
        _lastPassContactManifoldCount = 0;
        _passContactSince = 0;
        _lastPassContactAt = 0;
        ...
        StoppedObstaclePassCommitCount = 0;
        StoppedObstaclePassCompletedCount = 0;
        SpawnCounter++;
        Initialized = true;
        if (_configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race)
        {
            ...
            _gridLineMergeBlocked = _gridLineMergePending;
            _gridPathBlendStarted = !_gridLineMergePending;
            _gridPathBlend = _gridLineMergePending ? 0 : 1;
            _gridLaunchOrigin = pose.Position;
            _gridLaunchForward = Vector3.Transform(Vector3.UnitZ, pose.Orientation) with { Y = 0 };
            if (_gridLaunchForward.LengthSquared() < 1e-6f)
                _gridLaunchForward = startTangent;
            if (_gridLaunchForward.LengthSquared() > 1e-6f)
                _gridLaunchForward = Vector3.Normalize(_gridLaunchForward);
            ...
            {
                Log.Debug("Race bot {SessionId} grid launch heading differs from spline by {HeadingDifference:F1} degrees",
                    EntryCar.SessionId,
                    RaceBotMath.PlanarHeadingDifferenceDegrees(_gridLaunchForward, startTangent));
            }
    ...
    private (AiState? ClosestAiState, float ClosestAiStateDistance, float MaxSpeed) SplineLookahead()
    {
        ...
        bool raceMode = _configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race;
        float maxBrakingDistance = raceMode
            ? RaceBotMath.CornerBrakingDistance(CurrentSpeed, 0, EntryCar.AiDeceleration) + 20
            : PhysicsUtils.CalculateBrakingDistance(CurrentSpeed, EntryCar.AiDeceleration) * 2 + 20;
        ...
        if (raceMode)
        {
            float currentPointSpeedLimit = MathF.Sqrt(RaceBotMath.CorneringSpeedSquared(
                point.Radius, EntryCar.AiCorneringSpeedFactor,
                EntryCar.RaceDifficulty));
            maxSpeed = RaceBotMath.CornerApproachSpeedLimit(currentPointSpeedLimit, 0,
        ...
        while (distanceTravelled < maxBrakingDistance)
        {
            ...
            float maxCorneringSpeedSquared = raceMode
            ...
            if (raceMode)
            {
                // Keep a continuous braking envelope through the corner. The old planner only
                // retained a corner limit while the car was above it, so it reapplied throttle
                // before the bend was complete and failed repeatedly at the same spline points.
                float approachSpeedLimit = RaceBotMath.CornerApproachSpeedLimit(pointSpeedLimit,
                    distanceTravelled, EntryCar.AiDeceleration);
                maxSpeed = Math.Min(approachSpeedLimit, maxSpeed);
            }
            else if (pointSpeedLimit * pointSpeedLimit < currentSpeedSquared)
            {
    ...
    private bool TryGetRaceParticipant(EntryCar car, out RaceParticipantSnapshot snapshot)
    {
        ...
        if (car.AiControlled)
        {
            ...
            if (_configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race
                ...
                && !_racePhysicsWorld.IsBotRaceObstacleEligible(car.SessionId))
            {
                snapshot = default;
                return false;
            }
    ...
    private bool IsCurrentRacePassTargetEligible()
    {
        if (!_overtakeTargetSessionId.HasValue)
            return true;
        var targetCar = _entryCarManager.EntryCars.FirstOrDefault(car =>
            car.SessionId == _overtakeTargetSessionId.Value);
        return targetCar != null && TryGetRaceParticipant(targetCar, out _);
    }
    ...
    private RaceObstacle? FindClosestRaceObstacle()
    {
        ...
        float maximumLookahead = Math.Max(60,
            RaceBotMath.OvertakeTriggerDistance(CurrentSpeed,
                EntryCar.RaceAggression) * 1.5f);
        float explicitStopLookahead = RaceBotMath.PassAttemptDistance(CurrentSpeed, 0,
            EntryCar.RaceAggression) * 1.15f;
        var candidates = new List<RaceObstacleCandidate>();
        ...
        foreach (var car in _entryCarManager.EntryCars)
        {
            ...
            float participantLookahead = car.GetRaceControlMode()
                                           == RaceControlBotControlMode.Stopped
                ? Math.Max(maximumLookahead, explicitStopLookahead)
                : maximumLookahead;
            if (longitudinal <= 0 || longitudinal > participantLookahead)
                continue;
            ...
            if (Math.Abs(relativeLateral) > RaceBotMath.StoppedQueueLateralCorridorMeters)
                continue;
            ...
            float halfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(car.Model) ?? 1;
            candidates.Add(new RaceObstacleCandidate(participant, clearance, longitudinal,
                obstacleOffset, relativeLateral, halfWidth));
        }

        var ordered = candidates.OrderBy(candidate => candidate.LongitudinalMeters).ToArray();
        var nearest = ordered.FirstOrDefault(candidate =>
            Math.Abs(candidate.RelativeLateralMeters) <= RaceBotMath.ObstacleCorridorHalfWidthMeters);
        if (nearest.Participant.Car == null)
            return null;

        bool nearestStopped = RaceBotMath.IsStoppedObstacle(nearest.Participant.Speed);
        bool nearestExplicitlyStopped = nearest.Participant.Car.GetRaceControlMode()
                                        == RaceControlBotControlMode.Stopped;
        bool mayLeadToExplicitStop = nearestStopped
                                     || nearest.Participant.Speed
                                     < RaceBotMath.StoppedQueueApproachSpeedMetersPerSecond;
        if (!mayLeadToExplicitStop)
        {
            return new RaceObstacle(nearest.Participant, nearest.ClearanceMeters,
                nearest.LongitudinalMeters, nearest.LateralOffsetMeters,
                nearest.RelativeLateralMeters, nearest.Participant, nearest.LongitudinalMeters,
                nearest.LateralOffsetMeters - nearest.HalfWidthMeters,
                nearest.LateralOffsetMeters + nearest.HalfWidthMeters,
                IsStoppedCluster: false, ClusterSize: 1,
                ClusterSessionIds: [nearest.Participant.Car.SessionId]);
        }

        var queue = new List<RaceObstacleCandidate> { nearest };
        float lastLongitudinal = nearest.LongitudinalMeters;
        bool explicitStoppedCar = nearestExplicitlyStopped;
        if (!nearestExplicitlyStopped)
        {
            foreach (var candidate in ordered)
            {
                if (candidate.LongitudinalMeters <= lastLongitudinal)
                    continue;
                float gap = candidate.LongitudinalMeters - lastLongitudinal;
                bool candidateExplicitlyStopped = candidate.Participant.Car.GetRaceControlMode()
                                                  == RaceControlBotControlMode.Stopped;
                float linkedSpeedLimit = nearestStopped
                    ? RaceBotMath.StoppedObstacleSpeedMetersPerSecond
                    : RaceBotMath.StoppedQueueApproachSpeedMetersPerSecond;
                if (gap > RaceBotMath.StoppedQueueLinkDistanceMeters
                    || (!candidateExplicitlyStopped
                        && candidate.Participant.Speed >= linkedSpeedLimit))
                    break;
                queue.Add(candidate);
                lastLongitudinal = candidate.LongitudinalMeters;
                if (candidateExplicitlyStopped)
                {
                    explicitStoppedCar = true;
                    break;
                }
            }
        }

        if (!nearestStopped && !explicitStoppedCar)
        {
            return new RaceObstacle(nearest.Participant, nearest.ClearanceMeters,
                nearest.LongitudinalMeters, nearest.LateralOffsetMeters,
                nearest.RelativeLateralMeters, nearest.Participant, nearest.LongitudinalMeters,
        ...
        var passTarget = queue[^1];
        bool naturalRoot = passTarget.Participant.AiState == null
                           || passTarget.Participant.AiState.ClosestAiObstacleDistance < 0;
        bool stoppedCluster = explicitStoppedCar || queue.Count > 1 || naturalRoot;
        float minimumEdge = queue.Min(candidate =>
            candidate.LateralOffsetMeters - candidate.HalfWidthMeters);
        float maximumEdge = queue.Max(candidate =>
            candidate.LateralOffsetMeters + candidate.HalfWidthMeters);
        return new RaceObstacle(nearest.Participant, nearest.ClearanceMeters,
            nearest.LongitudinalMeters, nearest.LateralOffsetMeters,
            nearest.RelativeLateralMeters, passTarget.Participant,
            passTarget.LongitudinalMeters, minimumEdge, maximumEdge,
            stoppedCluster, queue.Count,
            queue.Select(candidate => candidate.Participant.Car.SessionId).ToArray());
    }

    private bool IsPassTargetOccupied(float targetOffsetMeters, byte passTargetSessionId,
        float obstacleLongitudinalMeters, bool stoppedObstacle)
    {
        var sample = GetCurrentSplineSample(out _);
        var forward = sample.Tangent with { Y = 0 };
        if (forward.LengthSquared() < 1e-6f)
        ...
        {
            if (car.SessionId == EntryCar.SessionId
                || car.SessionId == passTargetSessionId
                || !TryGetRaceParticipant(car, out var participant)
                || Math.Abs(participant.Status.Position.Y - Status.Position.Y) >= 3)
                continue;
            float longitudinal = GetRaceLongitudinalDistance(participant, forward);
            float rearReservation = RaceBotMath.PassLaneRearReservationDistance(
                CurrentSpeed, participant.Speed);
            if (!RaceBotMath.IsInsidePassLaneReservation(longitudinal, rearReservation,
                    obstacleLongitudinalMeters, stoppedObstacle))
                continue;
            var relative = participant.Status.Position - Status.Position;
            relative.Y = 0;
            float currentOffset = PhysicalLateralOffsetMeters
                                  + GetParticipantRelativeLateral(participant, relative, lateral);
            float participantHalfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(car.Model) ?? 1;
            float ownHalfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(EntryCar.Model) ?? 1;
            float requiredSeparation = RaceBotMath.RequiredPassSeparation(ownHalfWidth,
                participantHalfWidth, RaceBotMath.IsStoppedObstacle(participant.Speed));
    ...
    private float GetPredictivePassCorridorCost(float targetOffsetMeters,
        RaceObstacle obstacle, float ownHalfWidthMeters, float horizonSeconds,
        Vector3 forward, Vector3 lateral)
    {
        float cost = Math.Abs(targetOffsetMeters - PhysicalLateralOffsetMeters) * 0.35f
                     + Math.Abs(targetOffsetMeters) * 0.04f;
        float ownLength = EntryCar.VehicleLengthPreMeters + EntryCar.VehicleLengthPostMeters;
        float maximumLongitudinal = Math.Max(
            RaceBotMath.MaximumPredictivePassPlanningDistanceMeters + 40,
            obstacle.PassTargetLongitudinalMeters + 40);
        foreach (var car in _entryCarManager.EntryCars)
        {
            if (car.SessionId == EntryCar.SessionId
                || !TryGetRaceParticipant(car, out var participant)
            ...
            float longitudinal = GetRaceLongitudinalDistance(participant, forward);
            if (longitudinal < -RaceBotMath.MaximumPredictivePassPlanningDistanceMeters
                || longitudinal > maximumLongitudinal)
                continue;
            var relative = participant.Status.Position - Status.Position;
            relative.Y = 0;
            float participantOffset = PhysicalLateralOffsetMeters
                                      + GetParticipantRelativeLateral(participant,
                                          relative, lateral);
            float participantTargetOffset = participantOffset;
            if (participant.AiState is { Initialized: true } participantAi)
            {
                participantTargetOffset = participantAi._targetLateralOffsetMeters;
            }
            else
            {
                float lateralVelocity = Vector3.Dot(participant.Status.Velocity, lateral);
                participantTargetOffset += Math.Clamp(lateralVelocity * horizonSeconds,
                    -3f, 3f);
            }

            float participantHalfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(
                car.Model) ?? 1;
            float requiredSeparation = RaceBotMath.RequiredPassSeparation(
                ownHalfWidthMeters, participantHalfWidth,
                RaceBotMath.IsStoppedObstacle(participant.Speed))
                + (participant.AiState == null
                    ? RaceBotMath.PredictiveHumanUncertaintyMeters
                    : RaceBotMath.PredictivePassSafetyBufferMeters);
            float interactionCost = RaceBotMath.PredictiveCorridorInteractionCost(
                longitudinal, CurrentSpeed, participant.Speed,
                PhysicalLateralOffsetMeters, targetOffsetMeters,
                participantOffset, participantTargetOffset,
                ownLength, car.VehicleLengthPreMeters + car.VehicleLengthPostMeters,
                requiredSeparation, horizonSeconds);
            if (float.IsPositiveInfinity(interactionCost))
    ...
    private bool HasReliablePredictivePassSurface(float targetOffsetMeters,
        float horizonSeconds)
    {
        ...
        const float maximumSurfaceHeightErrorMeters = 0.75f;
        float planningDistance = Math.Min(RaceBotMath.MaximumPredictivePassPlanningDistanceMeters,
            Math.Max(30, CurrentSpeed * horizonSeconds));
        for (int sampleId = 1; sampleId <= sampleCount; sampleId++)
        {
            float progress = sampleId / (float)sampleCount;
            float elapsedSeconds = horizonSeconds * progress;
            float predictedOffset = RaceBotMath.PredictLateralOffset(
                PhysicalLateralOffsetMeters, targetOffsetMeters,
                elapsedSeconds, horizonSeconds);
            var target = GetRaceSteeringTarget(planningDistance * progress,
                predictedOffset);
            if (!_racePhysicsWorld.HasReliableTrackSurface(target,
                    maximumSurfaceHeightErrorMeters))
                return false;
        }
    ...
    private bool TrySelectLegacyMovingPassCorridor(RaceObstacle obstacle,
        (float SideLeft, float SideRight) corridor, float ownHalfWidthMeters,
        float requiredSeparationMeters, float trackEdgeReserveMeters,
        out float targetOffsetMeters)
    {
        ...
        float? target = RaceBotMath.ChoosePassTarget(_lateralOffsetMeters,
            ...
            requiredSeparationMeters, ownHalfWidthMeters, trackEdgeReserveMeters);
        if (!target.HasValue)
            return false;

        if (!RaceBotMath.HasRequiredPassTargetClearance(target.Value,
                obstacle.LateralOffsetMeters, requiredSeparationMeters)
            || IsPassTargetOccupied(target.Value, obstacle.PassTarget.Car.SessionId,
                obstacle.PassTargetLongitudinalMeters, stoppedObstacle: false))
        {
        ...
        targetOffsetMeters = target.Value;
        return true;
    }

    private bool TrySelectPredictivePassCorridor(RaceObstacle obstacle,
        (float SideLeft, float SideRight) corridor, float ownHalfWidthMeters,
        float requiredSeparationMeters, float trackEdgeReserveMeters,
        out float targetOffsetMeters, out float? alternateOffsetMeters,
        out float horizonSeconds, out int candidateCount, out float selectedCost)
    {
        targetOffsetMeters = 0;
        alternateOffsetMeters = null;
        selectedCost = float.PositiveInfinity;
        horizonSeconds = RaceBotMath.PredictivePassHorizonSeconds(CurrentSpeed,
            obstacle.Participant.Speed, EntryCar.RaceAggression);

        float maximumLeftPassCenter;
        float minimumRightPassCenter;
        if (obstacle.IsStoppedCluster)
        {
            float stoppedMargin = ownHalfWidthMeters
                                  + RaceBotMath.StoppedObstacleSafetyMarginMeters
                                  + RaceBotMath.StoppedObstacleTargetBufferMeters;
            maximumLeftPassCenter = obstacle.EnvelopeMinimumEdgeMeters - stoppedMargin;
            minimumRightPassCenter = obstacle.EnvelopeMaximumEdgeMeters + stoppedMargin;
        }
        else
        {
            maximumLeftPassCenter = obstacle.LateralOffsetMeters - requiredSeparationMeters
                                    - RaceBotMath.PredictivePassSafetyBufferMeters;
            minimumRightPassCenter = obstacle.LateralOffsetMeters + requiredSeparationMeters
                                     + RaceBotMath.PredictivePassSafetyBufferMeters;
        }

        var candidates = RaceBotMath.BuildPredictivePassCorridors(
            PhysicalLateralOffsetMeters, corridor.SideLeft, corridor.SideRight,
            ownHalfWidthMeters, trackEdgeReserveMeters,
            maximumLeftPassCenter, minimumRightPassCenter, EntryCar.SessionId);
        ...
        float envelopeCenter = (obstacle.EnvelopeMinimumEdgeMeters
                                + obstacle.EnvelopeMaximumEdgeMeters) * 0.5f;
        float bestLeftCost = float.PositiveInfinity;
        float bestRightCost = float.PositiveInfinity;
        float bestLeft = 0;
        float bestRight = 0;
        float maximumCenterOffset = RaceBotMath.MaximumPredictivePassCenterOffsetMeters(
            CurrentSpeed);
        foreach (float candidate in candidates)
        {
            if (Math.Abs(candidate) > maximumCenterOffset)
                continue;
            bool passingLeft = candidate < envelopeCenter;
            if (obstacle.IsStoppedCluster
                && HasConflictingStoppedPassReservation(obstacle, candidate,
                    requiredSeparationMeters))
                continue;
            float cost = GetPredictivePassCorridorCost(candidate, obstacle,
                ownHalfWidthMeters, horizonSeconds, forward, lateral);
            if (float.IsPositiveInfinity(cost))
                continue;
            if (passingLeft && cost < bestLeftCost
                && HasReliablePredictivePassSurface(candidate, horizonSeconds))
            {
                bestLeftCost = cost;
                bestLeft = candidate;
            }
            else if (!passingLeft && cost < bestRightCost
                     && HasReliablePredictivePassSurface(candidate, horizonSeconds))
            {
                bestRightCost = cost;
                bestRight = candidate;
        ...
        bool hasLeft = float.IsFinite(bestLeftCost);
        bool hasRight = float.IsFinite(bestRightCost);
        if (!hasLeft && !hasRight)
            return false;
        bool chooseLeft = hasLeft && (!hasRight || bestLeftCost < bestRightCost
            || (Math.Abs(bestLeftCost - bestRightCost) < 0.05f
                && (EntryCar.SessionId & 1) == 0));
        if (chooseLeft)
        {
            targetOffsetMeters = bestLeft;
            selectedCost = bestLeftCost;
            if (hasRight)
                alternateOffsetMeters = bestRight;
        }
        else
        {
            targetOffsetMeters = bestRight;
            selectedCost = bestRightCost;
            if (hasLeft)
                alternateOffsetMeters = bestLeft;
        }
        return true;
    }
    ...
    {
        int pointId = CurrentSplinePointId;
        float sideLeft = float.PositiveInfinity;
        float sideRight = float.PositiveInfinity;
        float distance = 0;
        int visited = 0;
        while ((uint)pointId < (uint)_spline.Points.Length
               && distance <= Math.Max(0, lookaheadMeters)
               && visited++ < _spline.Points.Length)
        {
            ref readonly var point = ref _spline.Points[pointId];
            sideLeft = Math.Min(sideLeft, Math.Max(0, point.SideLeft));
            sideRight = Math.Min(sideRight, Math.Max(0, point.SideRight));
            distance += Math.Max(0, point.Length);
            if (point.NextId < 0 || point.NextId == pointId)
                break;
            pointId = point.NextId;
        }

        ref readonly var current = ref _spline.Points[CurrentSplinePointId];
        return (float.IsFinite(sideLeft) ? sideLeft : current.SideLeft,
            float.IsFinite(sideRight) ? sideRight : current.SideRight);
    }
    ...
    private void UpdateBaseLaneOffset()
    {
        ...
        if (_gridLineMergePending)
        {
            ...
            if (!_gridPathBlendStarted
                && RaceBotMath.CanBeginGridPathBlend(
                    _sessionManager.CurrentSession.Configuration.Type,
                    _sessionManager.ServerTimeMilliseconds,
                    _sessionManager.CurrentSession.StartTimeMilliseconds,
                    _gridLaunchDistanceMeters))
            {
                _gridPathBlendStarted = true;
                Log.Debug("Race bot {SessionId} blending grid path into spline after {LaunchDistance:F1} m",
            ...
            _gridLineMergeBlocked = !_gridLineMergeStarted || corridorOccupied;
            if (_gridLineMergeBlocked)
            {
            ...
            _targetLateralOffsetMeters = _baseLateralOffsetMeters;
            if (_gridPathBlend >= 1
                && Math.Abs(_lateralOffsetMeters - _targetLateralOffsetMeters) < 0.05f)
            {
                _gridLineMergePending = false;
                _gridLineMergeBlocked = false;
            }
    ...
    private float GetRaceBaseLaneOffset(in SplinePoint point)
    {
        ...
        return RaceBotMath.RacingLineOffset(point.SideLeft, point.SideRight,
            _racePersonalitySeed, distance);
    }

    private int CountNearbyRaceParticipants(float radiusMeters)
    {
        float radius = Math.Max(0, radiusMeters);
        ...
        foreach (var car in _entryCarManager.EntryCars)
        {
            if (car.SessionId == EntryCar.SessionId
                ...
                || Math.Abs(participant.Status.Position.Y - Status.Position.Y) >= 3)
                continue;
            var difference = participant.Status.Position - Status.Position;
            difference.Y = 0;
    ...
    private bool TryBeginRaceOvertake(RaceObstacle obstacle, bool allowCloseContact = false)
    {
        ...
        if (_configuration.Extra.AiParams.Behavior != AiBehaviorMode.Race
            ...
            return false;
        if (RaceBotMath.IsStoppedObstacle(obstacle.Participant.Speed) && !stoppedObstacle)
            return false;
        if (!stoppedObstacle
            && !RaceBotMath.CanAttemptPassPair(passTarget.Car.SessionId,
        ...
        float corridorLookahead = stoppedObstacle
            ? Math.Min(obstacle.PassTargetLongitudinalMeters + 10,
                Math.Max(RaceBotMath.StoppedObstaclePassPlanningDistanceMeters,
                    CurrentSpeed * 2f))
            : obstacle.PassTargetLongitudinalMeters + 10;
        var corridor = GetPassCorridorWidths(corridorLookahead);
        float ownHalfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(EntryCar.Model) ?? 1;
        corridor = (RaceBotMath.LimitPassCorridorWidth(corridor.SideLeft, ownHalfWidth),
            RaceBotMath.LimitPassCorridorWidth(corridor.SideRight, ownHalfWidth));
        float obstacleHalfWidth = _racePhysicsWorld?.GetVehicleHalfWidthMeters(
        ...
        bool predictivePlanner = RaceBotMath.ShouldUsePredictiveCorridorPlanner(
            CurrentSpeed, obstacle.Participant.Speed);
        float target;
        float? alternateTarget = null;
        float predictionHorizon = 0;
        int predictionCandidates = 0;
        float predictionCost = 0;
        if (predictivePlanner)
        {
            if (!TrySelectPredictivePassCorridor(obstacle, corridor, ownHalfWidth,
                    requiredSeparation, trackEdgeReserve, out target,
                    out alternateTarget, out predictionHorizon,
                    out predictionCandidates, out predictionCost))
                return false;
        }
        else if (!TrySelectLegacyMovingPassCorridor(obstacle, corridor, ownHalfWidth,
                     requiredSeparation, trackEdgeReserve, out target))
        {
            return false;
        }

        if (!RaceBotMath.IsPracticalPassTarget(_lateralOffsetMeters, target,
                obstacle.Participant.Speed))
            return false;

        _targetLateralOffsetMeters = target;
        _committedPassLateralOffsetMeters = target;
        _alternatePassLateralOffsetMeters = alternateTarget;
        float envelopeCenter = (obstacle.EnvelopeMinimumEdgeMeters
                                + obstacle.EnvelopeMaximumEdgeMeters) * 0.5f;
        _passingLeft = target < envelopeCenter;
        _overtakeTargetSessionId = passTarget.Car.SessionId;
        _currentPassSeparationRecorded = false;
        _currentPassAccelerationClearanceRecorded = false;
        _passAccelerationClearanceSince = 0;
        _stoppedObstaclePass = stoppedObstacle;
        _requiredPassSeparationMeters = requiredSeparation;
        _passAccelerationSeparationMeters = RaceBotMath.PassAccelerationSeparation(
            requiredSeparation, stoppedObstacle);
        _overtakeExtensionCount = 0;
        _stoppedPassClusterSessionIds = stoppedObstacle ? obstacle.ClusterSessionIds : [];
        _lastPassContactManifoldCount = GetStoppedPassContactManifoldCount();
        _passContactSince = 0;
        _lastPassContactAt = 0;
        _blockedPassReverseUntil = 0;
        _blockedPassReplanCount = 0;
        ...
        PassCommitCount++;
        if (stoppedObstacle)
            StoppedObstaclePassCommitCount++;
        _overtakeUntil = _sessionManager.ServerTimeMilliseconds
                         + RaceBotMath.OvertakeCommitMilliseconds(
                             EntryCar.RaceAggression,
                             obstacle.ClearanceMeters);
        Log.Debug("Race bot {SessionId} committed {PassKind} pass of {TargetSessionId} at {Distance:F1} m: lane {Current:F2} -> {Target:F2} m, obstacle {Obstacle:F2} m, required separation {Required:F2} m, corridor L/R {Left:F2}/{Right:F2} m, cluster {ClusterSize}, planner {Planner}, prediction {PredictionHorizon:F1} s/{PredictionCandidates} candidates, cost {PredictionCost:F2}",
            EntryCar.SessionId, stoppedObstacle ? "stopped-obstacle" : "racing",
            passTarget.Car.SessionId, obstacle.ClearanceMeters,
            _lateralOffsetMeters, target, obstacle.LateralOffsetMeters, requiredSeparation,
            corridor.SideLeft, corridor.SideRight, obstacle.ClusterSize,
            predictivePlanner ? "predictive" : "high-speed",
            predictionHorizon, predictionCandidates, predictionCost);
        return true;
    }
    ...
    {
        bool stoppedObstaclePass = _stoppedObstaclePass;
        if (completed)
        {
            ...
            CompletedPassCount++;
            if (stoppedObstaclePass)
                StoppedObstaclePassCompletedCount++;
        ...
        _requiredPassSeparationMeters = RaceBotMath.MinimumPassingSeparationMeters;
        _passAccelerationSeparationMeters = RaceBotMath.MinimumPassAccelerationSeparationMeters;
        _committedPassLateralOffsetMeters = _targetLateralOffsetMeters;
        _alternatePassLateralOffsetMeters = null;
        _lastPassContactManifoldCount = 0;
        _passContactSince = 0;
        _lastPassContactAt = 0;
    ...
    {
        if (!TryGetOvertakeTargetWorldRelative(out var relative, out var longitudinal,
                out var lateral))
            return false;
        if (!_stoppedObstaclePass)
            return RaceBotMath.HasCompletedPass(_currentPassSeparationRecorded,
                relative.Length(), longitudinal, lateral);

        long now = _sessionManager.ServerTimeMilliseconds;
        if (_racePhysicsWorld != null && _overtakeTargetSessionId.HasValue)
        {
        ...
        if (!RaceBotMath.HasCompletedStoppedPass(_currentPassSeparationRecorded,
                longitudinal, lateral, _requiredPassSeparationMeters))
        {
            _stoppedPassAheadSince = 0;
            return false;
        }
        if (_stoppedPassAheadSince == 0)
            _stoppedPassAheadSince = now;
        return now - _stoppedPassAheadSince >= RaceBotMath.StoppedPassCompletionHoldMilliseconds
               && (_lastPassContactAt == 0
                   || now - _lastPassContactAt
    ...
    private bool UpdateStoppedPassContactRecovery()
    {
        ...
        {
            float previous = _committedPassLateralOffsetMeters;
            _committedPassLateralOffsetMeters = _alternatePassLateralOffsetMeters.Value;
            _alternatePassLateralOffsetMeters = previous;
            _targetLateralOffsetMeters = _committedPassLateralOffsetMeters;
            _passingLeft = _committedPassLateralOffsetMeters < PhysicalLateralOffsetMeters;
            _blockedPassReplanCount++;
            _passContactSince = now;
            _currentPassSeparationRecorded = false;
            _currentPassAccelerationClearanceRecorded = false;
            _passAccelerationClearanceSince = 0;
            _overtakeUntil = Math.Max(_overtakeUntil,
        ...
        _blockedPassReverseSteering = _passingLeft ? 0.65f : -0.65f;
        _blockedPassReplanCount++;
        _passContactSince = 0;
        _currentPassAccelerationClearanceRecorded = false;
        _passAccelerationClearanceSince = 0;
    ...
    private long GetStoppedPassContactManifoldCount()
    {
        if (_racePhysicsWorld == null)
            return 0;
        if (_stoppedPassClusterSessionIds.Length == 0)
        {
            return _overtakeTargetSessionId.HasValue
                ? _racePhysicsWorld.GetVehicleContactManifoldCount(EntryCar.SessionId,
                    _overtakeTargetSessionId.Value)
                : 0;
        }

        long count = 0;
        foreach (byte sessionId in _stoppedPassClusterSessionIds)
            count += _racePhysicsWorld.GetVehicleContactManifoldCount(EntryCar.SessionId, sessionId);
        return count;
    }
    ...
    public void DetectObstacles()
    {
        ...
        if (_configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race
            && EntryCar.GetRaceControlMode() == RaceControlBotControlMode.Stopped)
        {
            if (_overtakeTargetSessionId.HasValue)
                EndRaceOvertake(completed: false);
            SetTargetSpeed(0);
        ...
        if (RaceBotMath.ShouldStopAfterReportedCollision(
                _configuration.Extra.AiParams.Behavior)
        ...
        if (_configuration.Extra.AiParams.Behavior == AiBehaviorMode.Race)
        {
            ...
            if (_overtakeTargetSessionId.HasValue && !IsCurrentRacePassTargetEligible())
            {
                Log.Debug("Race bot {SessionId} abandoned pass of off-surface or recovering bot {TargetSessionId}",
                    EntryCar.SessionId, _overtakeTargetSessionId.Value);
                EndRaceOvertake(completed: false);
            }
                     ...
                     && _sessionManager.ServerTimeMilliseconds >= _overtakeUntil)
            {
                ...
                if (_currentPassSeparationRecorded
                    && RaceBotMath.ShouldExtendPass(longitudinalGap, CurrentSpeed,
                        overtakeTargetSpeed, EntryCar.RaceAggression,
            ...
            bool recentObstacleContact = raceObstacle.HasValue
                                         && _racePhysicsWorld?.WasVehicleContactRecent(
                                             EntryCar.SessionId,
                                             raceObstacle.Value.Participant.Car.SessionId) == true;
            if (recentObstacleContact)
                ContinueAfterCollision(raceObstacle!.Value.Participant.Car.SessionId);
            bool persistThroughContact = raceObstacle.HasValue
                                         && _raceContactTargetSessionId
                                         == raceObstacle.Value.Participant.Car.SessionId
                                         && _sessionManager.ServerTimeMilliseconds
            ...
            float aggression = EntryCar.RaceAggression;
            ...
            if (!committedOvertake && raceObstacle.HasValue
                && RaceBotMath.CanAttemptPass(_sessionManager.CurrentSession.Configuration.Type,
                    _sessionManager.ServerTimeMilliseconds,
                    _sessionManager.CurrentSession.StartTimeMilliseconds)
                && (persistThroughContact
                    || RaceBotMath.ShouldAttemptPass(CurrentSpeed,
                        raceObstacle.Value.Participant.Speed,
                        raceObstacle.Value.ClearanceMeters, aggression))
                && (!RaceBotMath.IsStoppedObstacle(raceObstacle.Value.Participant.Speed)
                    || rootStoppedObstacle))
            ...
            if (raceObstacle.HasValue
                ...
                < _requiredPassSeparationMeters
                && !(committedOvertake && _stoppedObstaclePass))
            {
                if (persistThroughContact)
                {
                    // Once cars are trading paint, braking to zero locks the field together.
                    // Keep race throttle available while the lateral path separates them.
                    targetSpeed = InitialMaxSpeed;
                }
                else
                {
                    // A race car may brake hard for an overlapping corridor, but commanding a
                    // complete stop creates a self-sustaining queue when the cars are already
                    // rubbing. Preserve crawl drive until a legal passing lane opens.
                    targetSpeed = RaceBotMath.CongestionCrawlTargetSpeed(
                        raceObstacle.Value.Participant.Speed,
                        raceObstacle.Value.ClearanceMeters,
                        CountNearbyRaceParticipants(30));
                    hasObstacle = true;
                }
            }
            else if (committedOvertake && raceObstacle.HasValue)
            {
                bool mayAccelerate = _currentPassAccelerationClearanceRecorded
                                     || (_stoppedObstaclePass
                                         && _currentPassSeparationRecorded);
                if (!mayAccelerate)
                {
            ...
            else if (committedOvertake)
            {
                ...
                {
                    float longitudinal = GetOvertakeTargetLongitudinalDistance() ?? 0;
                    float clearance = Math.Max(0, longitudinal
                                                  - EntryCar.VehicleLengthPreMeters
                                                  - (targetCar?.VehicleLengthPostMeters ?? 0));
                    targetSpeed = RaceBotMath.CommittedPassApproachSpeed(leadSpeed, clearance);
                    hasObstacle = true;
                }
            }
        ...
        if (yieldingToPasser)
            targetSpeed = RaceBotMath.YieldingTargetSpeed(targetSpeed, _yieldSpeedReference,
                GetSeparatedPasserSpeed(), EntryCar.RaceAggression);
        if (yieldingToPasser != _yieldingToPasser)
        {
    ...
    public void StopForCollision()
    {
        if (!RaceBotMath.ShouldStopAfterReportedCollision(
                _configuration.Extra.AiParams.Behavior)
            || ShouldIgnorePlayerObstacles())
            return;

        int duration = _random.Next(EntryCar.AiMinCollisionStopTimeMilliseconds,
            EntryCar.AiMaxCollisionStopTimeMilliseconds);
        _stoppedForCollisionUntil = _sessionManager.ServerTimeMilliseconds + duration;
    }

    public void ContinueAfterCollision(byte otherSessionId)
    {
        if (_configuration.Extra.AiParams.Behavior != AiBehaviorMode.Race)
            return;
        _raceContactTargetSessionId = otherSessionId;
        _raceContactPersistenceUntil = Math.Max(_raceContactPersistenceUntil,
            _sessionManager.ServerTimeMilliseconds
            + RaceBotMath.RaceContactPersistenceMilliseconds);
    }
    ...
    private Vector3 GetRaceSteeringTarget(float lookAheadMeters, float lateralOffsetMeters)
    {
        var sample = GetCurrentSplineSample(out int nextPointId);
        var cursor = sample.Position;
        int endPointId = nextPointId;
        float remaining = Math.Max(0, lookAheadMeters);
        int visited = 0;

        while ((uint)endPointId < (uint)_spline.Points.Length
               && visited++ < _spline.Points.Length)
        {
            var end = _spline.Points[endPointId].Position;
            var segment = end - cursor;
            float segmentLength = segment.Length();
            if (segmentLength > 1e-5f && remaining <= segmentLength)
            {
                var tangent = segment / segmentLength;
                var lateral = Vector3.Cross(Vector3.UnitY, tangent);
            ...
            cursor = end;
            int followingPointId = _spline.Points[endPointId].NextId;
            if (followingPointId < 0 || followingPointId == endPointId)
                break;
            endPointId = followingPointId;
        }
    ...
    public void PrepareRacePhysics(float fixedDeltaSeconds)
    {
        ...
        if (!holdForRaceControl && RaceBotMath.CanTransitionLane(_sessionManager.CurrentSession.Configuration.Type,
                currentTime, _sessionManager.CurrentSession.StartTimeMilliseconds))
        {
            float transitionMultiplier = _overtakeTargetSessionId.HasValue ? 4
                : _gridLineMergePending ? RaceBotMath.RaceGridMergeTransitionMultiplier : 1;
            _lateralOffsetMeters = _gridLineMergePending
                ? RaceBotMath.AdvanceLaneOffset(_lateralOffsetMeters,
                    _targetLateralOffsetMeters, CurrentSpeed, fixedDeltaSeconds,
                    transitionMultiplier)
                : RaceBotMath.AdvanceReachableLaneOffset(_lateralOffsetMeters,
                    _targetLateralOffsetMeters, PhysicalLateralOffsetMeters, CurrentSpeed,
                    fixedDeltaSeconds, transitionMultiplier, _stoppedObstaclePass);
        }
        ...
        var targetForward = tangent;
        if (_gridLineMergePending)
        {
            if (_gridPathBlendStarted)
            {
                _gridPathBlend = RaceBotMath.AdvanceGridPathBlend(_gridPathBlend,
                    CurrentSpeed, fixedDeltaSeconds);
            }
            var launchTarget = RaceBotMath.GridLaunchLineTarget(_gridLaunchOrigin,
                _gridLaunchForward, _physicsLastPosition, sample.Position.Y);
            targetPosition = Vector3.Lerp(launchTarget, targetPosition, _gridPathBlend);
            targetForward = RaceBotMath.BlendGridLaunchForward(_gridLaunchForward,
                tangent, _gridPathBlend);
        }
        float courseErrorMeters = Math.Abs(PhysicalLateralOffsetMeters - _lateralOffsetMeters);
        ref readonly var coursePoint = ref _spline.Points[CurrentSplinePointId];
        float vehicleHalfWidth = _racePhysicsWorld.GetVehicleHalfWidthMeters(EntryCar.Model);
        float courseBoundaryErrorMeters = _gridLineMergePending
            ? 0
            : RaceBotMath.CourseBoundaryErrorMeters(PhysicalLateralOffsetMeters,
                coursePoint.SideLeft, coursePoint.SideRight, vehicleHalfWidth, 0.15f);
        float steeringLateralOffsetMeters = _lateralOffsetMeters;
        if (courseBoundaryErrorMeters > 0)
        {
            steeringLateralOffsetMeters = RaceBotMath.ClampLaneOffset(
                steeringLateralOffsetMeters, coursePoint.SideLeft, coursePoint.SideRight,
                vehicleHalfWidth,
                RaceBotMath.OffroadRecoveryEdgeReserveMeters(courseBoundaryErrorMeters));
        }
        float steeringLookAhead = RaceBotPhysicsWorld.GetSteeringLookAheadMeters(CurrentSpeed,
            courseErrorMeters, courseBoundaryErrorMeters);
        var steeringTargetPosition = _gridLineMergePending
            ? targetPosition + targetForward * steeringLookAhead
            : GetRaceSteeringTarget(steeringLookAhead, steeringLateralOffsetMeters);
        ...
        else if (!hold && raceControlMode == RaceControlBotControlMode.Automatic
                       && currentTime < _blockedPassReverseUntil)
        {
            manualSteering = _blockedPassReverseSteering;
            manualAcceleration = -EntryCar.RaceVehicleProfile.MaxBrakeDeceleration * 0.45f;
            reverseRecovery = true;
        }
        _racePhysicsWorld.SetBotControl(EntryCar.SessionId, new RaceBotPhysicsControl(hold,
            stopForRaceControl,
            targetPosition, steeringTargetPosition, targetForward, TargetSpeed,
            ...
            manualSteering, manualAcceleration, reverseRecovery,
            CourseBoundaryErrorMeters: courseBoundaryErrorMeters));
    ...
    public void CompleteRacePhysics(float fixedDeltaSeconds)
    {
        ...
        if (_gridLineMergePending)
        {
            _gridLaunchDistanceMeters = Math.Max(_gridLaunchDistanceMeters,
                RaceBotMath.GridLaunchDistance(_gridLaunchOrigin, _gridLaunchForward,
        ...
        if (currentForward.LengthSquared() > 1e-6f)
        {
            ...
            if (_overtakeTargetSessionId.HasValue)
            {
                ...
                bool closeEnoughForAcceleration = hasWorldRelative
                                                  && relative.LengthSquared()
                                                  <= (_stoppedObstaclePass
                                                      ? RaceBotMath.StoppedObstaclePassPlanningDistanceMeters
                                                      * RaceBotMath.StoppedObstaclePassPlanningDistanceMeters
                                                      : RaceBotMath.PassSeparationEvaluationDistanceMeters
                                                      * RaceBotMath.PassSeparationEvaluationDistanceMeters)
                                                  && longitudinalGap
                ...
                if (!_currentPassSeparationRecorded
                    ...
                    && separation >= _requiredPassSeparationMeters)
                {
                ...
                if (!_currentPassAccelerationClearanceRecorded && closeEnoughForAcceleration)
                {
                    if (RaceBotMath.ShouldResetPassAccelerationClearance(separation,
                            _passAccelerationSeparationMeters))
                        _passAccelerationClearanceSince = 0;
